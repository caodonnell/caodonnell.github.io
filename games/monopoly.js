...
function Game() {
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Auction <span id='propertyname'></span></div><div>Highest Bid = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, it is your turn to bid (multiples of $10).</div<div><input id='bid' title='Enter an amount (multiples of $10) to bid on " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Bid' onclick='game.auctionBid();' title='Place your bid.' /><input type='button' value='Pass' title='Skip bidding this time.' onclick='game.auctionPass();' /><input type='button' value='Exit Auction' title='Stop bidding on " + s.name + " altogether.' onclick='if (confirm(\"Are you sure you want to stop bidding on this property altogether?\")) game.auctionExit();' /></div>", "blank");
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			document.getElementById("bid").value = "Please enter a bid (multiple of $10).";
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		if (initiator.communityChestJailCard) {
			...
			currentTableCellCheckbox.title = "Check this box to include this Get Out of Re-certification Free Card in the trade.";
			...
			currentTableCell.textContent = "Get Out of Re-certification Free Card";
		...
		if (initiator.chanceJailCard) {
			...
			currentTableCell.textContent = "Get Out of Re-certifcation Free Card";
	...
	this.acceptTrade = function(tradeObj) {
		...
		if (tradeObj.getCommunityChestJailCard() === 1) {
			...
			addAlert(recipient.name + ' received a "Get Out of Re-certification Free" card from ' + initiator.name + ".");
		} else if (tradeObj.getCommunityChestJailCard() === -1) {
			initiator.communityChestJailCard = true;
			recipient.communityChestJailCard = false;
			addAlert(initiator.name + ' received a "Get Out of Re-certification Free" card from ' + recipient.name + ".");
		}
...
function updateOwned() {
	...
	if (p.communityChestJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Re-certification Free Card</td></tr>";
	...
	if (p.chanceJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Get Out of Re-certification Free Card</td></tr>";
	}
...
function gotojail() {
	var p = player[turn];
	addAlert(p.name + " was sent directly to re-certification.");
	document.getElementById("landed").innerHTML = "You are in re-certification.";
...
function payfifty() {
	...
	addAlert(p.name + " paid the $500 fee to complete re-certification.");
	updateMoney();
	updatePosition();
}
...
function useJailCard() {
	...
	addAlert(p.name + " used a \"Get Out of Re-certification Free\" card.");
	updateOwned();
	updatePosition();
}
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (p.communityChestJailCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Get Out of Re-certification Free Card</td></tr>";
...
function land(increasedRent) {
	...
	if (p.position === 30) {
		...
		if (p.human) {
			popup("<div>Go directly to Re-certification. Do not pass GO. Do not collect $2000.</div>", gotojail);
		} else {
			gotojail();
		}
...
function roll() {
	...
	if (die1 == die2 && !p.jail) {
		...
		// If player rolls doubles three times in a row, send him to jail
			...
			if (p.human) {
				popup("You rolled doubles three times in a row. Go to Re-certification.", gotojail);
			} else {
				gotojail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			addAlert(p.name + " rolled doubles to complete re-certification.");
			...
			if (p.jailroll === 3) {
				...
				if (p.human) {
					popup("<p>You must pay the $500 fee.</p>", function() {
						payfifty();
...
function play() {
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "You are in Re-certification.<input type='button' title='Pay $500 to complete re-certification immediately.' value='Pay $500 fine' onclick='payfifty();' />";

		if (p.communityChestJailCard || p.chanceJailCard) {
			document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Use &quot;Get Out of Re-certification Free&quot; card.' onclick='useJailCard();' value='Use Card' />";
		}

		document.getElementById("nextbutton").title = "Roll the dice. If you throw doubles, you will complete re-certification.";

		if (p.jailroll === 0)
			addAlert("This is " + p.name + "'s first turn in re-certification.");
		else if (p.jailroll === 1)
			addAlert("This is " + p.name + "'s second turn in re-certification.");
		else if (p.jailroll === 2) {
			document.getElementById("landed").innerHTML += "<div>NOTE: If you do not throw doubles after this roll, you <i>must</i> pay the $500 fee.</div>";
			addAlert("This is " + p.name + "'s third turn in re-certification.");
		}
...
window.onload = function() {
	...
	document.getElementById("enlarge38token").innerHTML += '<img src="images/insurance_icon.png" height="60" width="70" alt="" style="position: relative; top: -20px;" />';

	corrections();

	// Jail corrections
	$("<div>", {id: "jailpositionholder" }).appendTo("#jail");
	$("<span>").text("Re-certification").appendTo("#jail");

	document.getElementById("jail").enlargeId = "enlarge40";

	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Re-certification</div><br /><div id='enlarge40price' class='enlarge-price'><img src='images/jake_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";

	document.getElementById("enlarge40name").innerHTML = "Re-certification";
